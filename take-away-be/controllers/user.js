...
userRouter.get('/restaurantDetails',userAuthFilter,async(req,res)=>{
    try{
        ...
        const restaurant = await RestaurantDetails.findOne({owner : req.params.token.id})
...
userRouter.post('/restaurantDetails',userAuthFilter,async(req,res)=>{
    try{
        ...
        const restaurantDetail = await RestaurantDetails.findOne({owner:req.params.token.id})
        if(restaurantDetail){
            const updatedDetails = await RestaurantDetails.findOneAndUpdate({owner : req.params.token.id},
                {
                    $set :req.body
                },
                {new:true}
...
userRouter.get('/ownerDetails',userAuthFilter,async(req,res)=>{
    try{
        const restaurant = await OwnerDetails.findOne({owner : req.params.token.id})
...
userRouter.post('/ownerDetails',userAuthFilter,async(req,res)=>{
    try{
        const ownerDetails =await OwnerDetails.findOne({owner:req.params.token.id})
        if(ownerDetails){
            const updatedOwnerDetails = await OwnerDetails.findOneAndUpdate({owner : req.params.token.id},
                {
                    $set : req.body
                },{new:true})
