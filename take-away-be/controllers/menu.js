...
menuRouter.post('/', userAuthFilter, async (req, res)=>{
    try{
        ...
        console.log(savedItem)
         await RestaurantDetails.findOneAndUpdate({owner:req.params.token.id},{$push:{menu:savedItem._id}})
...
menuRouter.put('/:menuId', userAuthFilter, async (req, res)=>{
    try{
        const updatedItem = await MenuDetails.findByIdAndUpdate({_id : req.params.menuId},
...
menuRouter.delete('/:menuId', userAuthFilter, async (req, res)=>{
    try{
        const deletedItem = await MenuDetails.findByIdAndRemove({_id : req.params.menuId})
...
menuRouter.get("/:restaurantId",userAuthFilter,async(req,res)=>{
    try {
        const menuItems = await MenuDetails.find({restaurant:req.params.restaurantId})
