    ...
    //   const [city,setCity] = useState("")
      ...
      useEffect(() =>{
            ...
            const getAddress = async()=>{
                try {
                    const data = await axios.get('http://localhost:4000/restaurant/customerUser/customer',{
                        headers: {
                            "Access-Control-Allow-Origin" : "*",
                            'Content-Type': 'application/json',
                            'Authorization' : token
                    ...
                    if(res){
                        setAddress(res[0] )
                        // setPhone(res.map(a=>a.phone))
                        // setDoorNum(res.map(a=>a.doorNum))
                        // setCity(res.map(a=>a.city))
                        // setStreet(res.map(a=>a.street))
                        // setPostCode(res.map(a=>a.postCode))
                    }
                } catch (error) {
                    console.log(error)
                }
          ...
          <div className={styles['form-control']}>
                 <label htmlFor='street'>Street Name</label>
                  <input 
                      ...
                      onBlur={formik.handleBlur}
                              />
                    {formik.touched.street&& formik.errors.street?<p className={styles.error}>{formik.errors.street}</p>:null}
                    </div>
           ...
           <div className={styles['form-control']}>
                 <label htmlFor='door'>Door Number</label>
                  <input 
                      ...
                      onBlur={formik.handleBlur}
                              />
                    {formik.touched.door&& formik.errors.door?<p className={styles.error}>{formik.errors.door}</p>:null}
                    </div>
                 ...
                 <label htmlFor='postCode'>Post Code</label>
                  <input 
                      ...
                      onBlur={formik.handleBlur}
                              />
                    {formik.touched.postCode&& formik.errors.postCode?<p className={styles.error}>{formik.errors.postCode}</p>:null}
                    </div>
                 ...
                 <label htmlFor='city'>City</label>
                  <input 
                      ...
                      onBlur={formik.handleBlur}
                              />
                    {formik.touched.city&& formik.errors.city?<p className={styles.error}>{formik.errors.city}</p>:null}
                    </div>
           ...
           <div className={classes.btn}>
            <button type='submit' onClick={onSubmit}>Order</button>
           </div>
