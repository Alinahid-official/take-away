      ...
      setBackdrop(!open);
       ...
       useEffect(()=>{
        ...
        async function getOwnerDetails(){
          try{
            setLoading(false)
            const response = await axios.get("http://localhost:4000/restaurant/ownerDetails",
            {
              headers:{
       ...
       const  onSubmit = async(values,actions)=>{
        ...
        try {
          setLoading(true)
          const response = await axios.post("http://localhost:4000/restaurant/ownerDetails",
  ...
  return(
    <div>
        <form onSubmit={formik.handleSubmit}>
          ...
          />
        {formik.touched.ownerNo && formik.errors.ownerNo?<p className={classes.error}>{formik.errors.ownerNo}</p>:null}
    </div>
    ...
    <div className={classes['form-control']}>
        ...
        onBlur={formik.handleBlur}
          />
        {formik.touched.ownerName && formik.errors.ownerName?<p className={classes.error}>{formik.errors.ownerName}</p>:null}
    </div>
   <div></div>
